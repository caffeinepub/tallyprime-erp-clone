...
// Auto check expiring subscriptions (runs daily at 9am)
cron.schedule('0 9 * * *', async () => {
  try {
    const [expiring] = await db.query(`
      SELECT s.*, u.email, u.phone, u.full_name, u.username
      FROM admin_subscriptions s JOIN users u ON u.id=s.user_id
      WHERE s.status='active' AND DATEDIFF(s.expiry_date, NOW()) BETWEEN 1 AND 3
    `);
...
// POST /api/reminders/send-manual - Super Admin sends manual reminder
router.post('/send-manual', superAdminAuth, async (req, res) => {
  try {
    const { user_id, message, channels } = req.body; // channels: ['email','sms','whatsapp']
    const [users] = await db.query('SELECT * FROM users WHERE id=?', [user_id]);
    if (!users.length) return res.status(404).json({ error: 'User not found' });
    const user = users[0];
    const sent = [];
    if (channels.includes('email') && user.email) {
      await sendEmail(user.email, 'Message from HisabKitab Pro Admin', `<p>${message}</p>`);
      sent.push('email');
    }
    ...
    await db.query('INSERT INTO notification_logs(user_id,type,channel,message,sent_by,status) VALUES(?,"manual",?,?,?,"sent")',
      [user_id, sent.join(','), message, req.superAdmin.username]);
    res.json({ message: 'Reminder sent', channels_used: sent });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// POST /api/reminders/broadcast - Super Admin broadcasts to all admins
  ...
  try {
    ...
    for (const admin of admins) {
      if (channels.includes('email') && admin.email) {
        await sendEmail(admin.email, subject || 'HisabKitab Pro Announcement', `<p>Dear ${admin.full_name},</p><p>${message}</p>`);
        emailCount++;
      }
      if (channels.includes('sms') && admin.phone) {
        await sendSMS(admin.phone, message);
        smsCount++;
      }
      await db.query('INSERT INTO notification_logs(user_id,type,channel,message,sent_by,status) VALUES(?,"broadcast",?,?,?,"sent")',
        [admin.id, channels.join(','), message, req.superAdmin.username]);
    }
    res.json({ message: 'Broadcast sent', emails_sent: emailCount, sms_sent: smsCount, total_admins: admins.length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// GET /api/reminders/logs - Notification logs (Super Admin)
...
// GET /api/reminders/my-notifications - User's own notifications

...
const SAMPLE_VOUCHERS: PendingVoucher[] = [
  {
    ...
    status: "pending",
    submittedAt: new Date(now.getTime() - 26 * 3600000).toISOString(),
  },
  {
    id: "pv2",
    date: "29 Mar 2026",
    ...
    status: "pending",
    submittedAt: new Date(now.getTime() - 5 * 3600000).toISOString(),
  },
  {
    id: "pv3",
    date: "30 Mar 2026",
    ...
    status: "pending",
    submittedAt: new Date(now.getTime() - 2 * 3600000).toISOString(),
  },
];
...
function isSLABreached(iso: string): boolean {
  return Date.now() - new Date(iso).getTime() > 24 * 3600 * 1000;
}

type FilterType = "all" | "pending" | "sla" | "escalated";
...
export default function MakerChecker() {
  ...
  const filteredPending = pending.filter((v) => {
    if (pendingFilter === "sla") return isSLABreached(v.submittedAt);
    if (pendingFilter === "escalated") return !!v.escalated;
    if (pendingFilter === "pending") return true;
    return true;
  });
    ...
    const updated = vouchers.map((v) =>
      v.id === id
        ? {
            ...
            approveRemarks: approveRemarks || undefined,
          }
        : v,
    );
    setVouchers(updated);
  ...
  const handleEscalate = (id: string) => {
    const updated = vouchers.map((v) =>
      v.id === id ? { ...v, escalated: !v.escalated } : v,
    );
    setVouchers(updated);
    savePending(updated);
    toast.success("Escalation flag toggled");
  };
      ...
      <Tabs defaultValue="pending" className="w-full">
        ...
        <TabsContent value="pending" className="mt-3 space-y-3">
          ...
          <div className="flex gap-1 flex-wrap">
            {(["all", "pending", "sla", "escalated"] as FilterType[]).map(
              (f) => {
                const label =
                  f === "all"
                    ? "All"
                ...
                return (
                  <button
                    ...
                    data-ocid={`maker_checker.filter_${f}.tab`}
          ...
          {filteredPending.length === 0 ? (
            ...
            <div className="space-y-3" data-ocid="maker_checker.table">
              {filteredPending.map((v, i) => {
                const sla = isSLABreached(v.submittedAt);
                return (
                  <div
                    key={v.id}
                    ...
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {sla && (
                            <Badge className="text-[9px] px-1.5 py-0 bg-orange-500/20 text-orange-400 border-orange-500/40">
                        ...
                        <div className="text-[11px] text-muted-foreground">
                          ...
                          <span
                            className={
                              sla ? "text-orange-400" : "text-muted-foreground"
                            }
                          >
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        <button
                          ...
                          data-ocid={`maker_checker.escalate.${i + 1}.toggle`}
                          className={`text-[10px] mt-1 flex items-center gap-1 transition-colors ${v.escalated ? "text-red-400" : "text-muted-foreground hover:text-red-400"}`}
                        >
                          <Flag size={10} />
                          {v.escalated ? "Escalated" : "Escalate"}
                        </button>
                      </div>
                    </div>

                    {rejectId === v.id ? (
                      <div className="flex gap-2 items-end border-t border-border/50 pt-3 flex-wrap">
                      ...
                      <div className="flex gap-2 items-end border-t border-border/50 pt-3 flex-wrap">
                        <div className="flex-1 space-y-1">
                          ...
                          <Input
                            ...
                            className="h-7 text-xs"
                            data-ocid={`maker_checker.approve_remarks.${i + 1}`}
                            autoFocus
                          />
                        </div>
                        <Button
                          ...
                          data-ocid={`maker_checker.confirm_approve.${i + 1}.primary_button`}
                        >
                        ...
                        <Button
                          ...
                          data-ocid={`maker_checker.cancel_approve.${i + 1}.cancel_button`}
                        >
                          Cancel
                        </Button>
                      </div>
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <p className="text-[10px] text-muted-foreground">
                  Vouchers above this amount need Admin approval
                </p>
              </div>
              <div className="space-y-1">

...
export default function VendorMaster({ company }: { company: Company }) {
  const key = `hkp-vendors-${company.id}`;
  ...
  const save = () => {
    ...
    if (editId) {
      setVendors((v) =>
        v.map((x) => (x.id === editId ? { ...form, id: editId } : x)),
      );
    } else {
      setVendors((v) => [...v, { ...form, id: Date.now().toString() }]);
    }
    setForm(empty());
    setEditId(null);
    setShowForm(false);
  ...
  const del = (id: string) => setVendors((v) => v.filter((x) => x.id !== id));

  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-[14px] font-bold text-foreground">Vendor Master</h2>
        <button
          type="button"
          ...
          <div className="grid grid-cols-3 gap-3">
            {(
              ...
              <div key={field}>
                ...
                <input
                  type={type}
                  data-ocid={field === "name" ? "vendor.input" : undefined}
            ...
            {vendors.map((v, i) => (
              ...
              >
                ...
                <td className="px-3 py-2 text-[12px] text-right">
                  ₹{v.creditLimit.toLocaleString("en-IN")}
                </td>
                ...
                <td className="px-3 py-2">
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${v.status === "Active" ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200" : "bg-gray-100 text-gray-600"}`}
                  >
                    {v.status}
                  </span>
                </td>
